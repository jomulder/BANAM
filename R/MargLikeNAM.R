...
#  if(is.null(priortype)){
  ...
  row.names(BFtu_exploratory) <- rhonames
  ...
  if(!is.null(hypothesis)){
    ...
    hypotheses <- names(BFta_confirmatory)
  }else{
  ...
  BF_out <- list(
    ...
    model=x,
    bayesfactor="Bayes factor based on normal priors",
    parameter="network autocorrelations",
    call=match.call())
